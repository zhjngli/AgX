// Algorithm: linear Rec.2020 -> gamma Rec.2020 transfer (sign-preserving sRGB curve)
// Canonical explanation: docs/book/src/reference/concepts/color-spaces.md
// CPU equivalent: crate::color_space::srgb_curve_signed (per channel)
...
fn main(@builtin(global_invocation_id) id: vec3u, @builtin(num_workgroups) nwg: vec3u) {
    ...
    let result = common::color::linear_to_gamma(rgb);
