...
struct Params {
    ...
    cg_balance_factor: f32, cg_balance_active: f32, _pad2: vec2f,
    vignette_amount: f32, vignette_shape: f32, _pad3: vec2f,
...
fn main(@builtin(global_invocation_id) id: vec3u) {
    ...
    let half = i32(params.dehaze_filter_radius);
    let x = i32(idx % w);
    let y = i32(idx / w);
    ...
    if params.dehaze_mode < 0.5 {
        // Horizontal
        let left = max(x - half, 0);
        let right = min(x + half, i32(w) - 1);
        for (var i = left; i <= right; i = i + 1) {
            min_val = min(min_val, input[u32(y) * w + u32(i)]);
        }
    } else {
        // Vertical
        let top = max(y - half, 0);
        let bottom = min(y + half, i32(h) - 1);
        for (var j = top; j <= bottom; j = j + 1) {
            min_val = min(min_val, input[u32(j) * w + u32(x)]);
        }
    }
