...
`preset` can depend on `lut` (for `Lut3D`) and `engine` (for `Parameters`), but the reverse is forbidden. Presets are consumers of those types: a TOML document declares values and produces a `Parameters` instance plus an optional `Lut3D`. Letting `engine` import from `preset` would invert the relationship and burden it with knowledge of a serialization format.
