...
- **LUT after per-pixel adjustments, before detail.** The LUT applies a creative color transform; sharpening and clarity then operate on the graded result. Sharpening before the LUT would amplify edges that the LUT would then re-grade.
...
- [LUT encoding](lut-encoding.md) — why LUTs sample in a separate space and how to choose between encodings.
