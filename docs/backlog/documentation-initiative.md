...
- [ ] **#6 Tutorials** — install guide; "Edit your first photo with the CLI"; "Apply a look to a whole directory with `batch-apply`"; "Compare multiple looks on one image with `multi-apply`". Uses existing `example/images/` and e2e presets. Blocked on design doc. Tutorials should link back to the relevant algorithm explanations from #4 (preset recipes ↔ "what is this slider doing"); when these land, backfill a `## See also` pointer from each algo page to the tutorial that uses it.
- [ ] **#7 How-to guides** — "Create your own preset from scratch"; "Extend an existing preset"; "Write and load a custom `.cube` LUT"; "Compose layered looks"; "Match the output of another preset". Blocked on design doc. Same cross-linking note as #6 — how-to guides cite algorithm explanations for theory; algo pages point forward to relevant how-tos once they exist.
...
- [ ] **#10 Markdown linting / formatting** *(follow-up from #4)* — add a `markdownlint` (or `prettier --prose-wrap`) step to `verify.sh` and the `docs-matrix` CI rollup. Pick a config (line length, trailing whitespace, list-style consistency, fenced-code-block language hints) and document it in `docs/contributing/documentation-conventions.md`. Source-style normalization only; no rendered-output change.
- [ ] **#11 Algorithm-page diagrams** *(follow-up from #4)* — author Mermaid diagrams for the heavier algorithms whose pipelines are hard to visualize from prose alone. Initial scope: dehaze (dark-channel → airlight → transmission → guided filter → recovery flow), denoise (à trous wavelet pyramid), color grading (lift / gamma / gain wheel layout), tone curves (Fritsch-Carlson tangent-limiter visualization). Mermaid is already wired in mdbook via `mdbook-mermaid`. Skip simple algorithms (exposure, white_balance, vignette) — prose suffices.
