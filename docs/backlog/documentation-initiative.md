...
- [x] **#7 How-to guides** — Six recipes: apply preset to folder (`batch-apply`), compare looks side-by-side (`multi-apply`), write a preset, extend a preset, author a custom `.cube` LUT, compose layered looks. "Match the output of another preset" cut and deferred to a future Recipes sub-project (genre-driven). Cross-link backfill from each `docs/book/src/explanation/*.md` to relevant how-tos lands in the same PR. Design: [Tutorials and How-to design](../plans/2026-04-26-tutorials-and-how-to-design.md).
- [x] **#8 Prose explanations** — three new explanation pages (architecture, preset-first philosophy, design decisions); split of three concept pages (render-pipeline, preset-model, color-spaces) into reference + paired explanation; "How AgX generates its bundled LUTs" prose moved out of `lut-format.md` into a standalone explanation page; `explanation/` reorganized into `concepts/` and `algorithms/` sub-sections; `documentation-conventions.md` extended with five Diataxis-derived authoring rules; `README.md` and `ARCHITECTURE.md` trimmed to summaries plus pointers. Design: [Prose explanations design](../plans/2026-04-27-prose-explanations-design.md).
