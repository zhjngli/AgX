...
- [ ] **HEIF encode (future)** — write `.heic` (HEVC codec, Apple-compatible, patent-encumbered) or `.avif` (AV1 codec, royalty-free) outputs. Codec choice deferred until concrete demand. Decode lands first; encode is a separate effort with its own licensing review. When this lands, it inherits the encode-time ICC contract from sub-project 2 in [color-management.md](color-management.md) — write the same `SRGB_V4_ICC` blob into the HEIF colr box / nclx tag.
