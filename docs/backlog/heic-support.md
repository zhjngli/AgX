...
- **EXIF orientation tag double-application on output.** AgX rotates pixel data per source EXIF orientation during decode (the README's "automatic EXIF orientation correction" feature), but the metadata-preservation path then copies the source orientation tag *unmodified* to the output file. Viewers that respect EXIF orientation (GitHub image previews, mdbook browser, macOS Preview in some configurations) rotate the already-canonical pixels a second time, producing visibly rotated images. Real fix: at encode time, after applying orientation to pixel data, reset the preserved EXIF orientation tag to `Horizontal (normal)` (orientation=1) or strip the tag. Affects all source formats that can carry an orientation tag (HEIC, JPEG, raw via embedded EXIF); HEIC is where users actually hit it because iPhones routinely capture with orientation≠1.
