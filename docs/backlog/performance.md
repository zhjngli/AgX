...
- [ ] **P6: SIMD for per-pixel adjustments** — vectorize the inner per-pixel loop with explicit SIMD. Additional 2-4x on top of parallelization. High complexity (sRGB-shape gamma `pow` needs fast approximation; applied to Rec.2020 linear values after the wide-working-space migration, the curve shape is unchanged). Also covers SIMD of the linear Rec.2020 ↔ linear sRGB boundary matrices flagged by the SP1 perf measurement.
