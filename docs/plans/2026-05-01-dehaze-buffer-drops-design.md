...
- **Block scoping.** Wrap each producer-and-mean pair in `let mean_x = { let x = ...; box_filter_2d(&x, ...) };` so `x` drops at the closing brace. Same MIR-level outcome as explicit `drop()`. For `gp` and `gg` this is syntactically feasible — they are produced sequentially, not interleaved — but it buries the multi-line fill closure and the box-filter call inside a block expression, making the two-step pattern harder to scan. For `a` and `b` the structural argument bites harder: they are populated together inside a single parallel `.zip()` closure because each pixel needs both values from shared intermediates, so block scoping would force restructuring the algorithm or duplicating that closure body across two blocks.
