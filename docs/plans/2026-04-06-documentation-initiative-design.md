...
| 4 | **Algorithm explanations** | **Complete.** Sibling `.md` file next to each `crates/agx/src/adjust/*.rs` (grain, dehaze, denoise, detail, color grading, tone curves, vignette, per-pixel adjustments) contains the canonical prose. `#![doc = include_str!("module.md")]` wiring on the Rust side and `{{#include ...}}` wiring in `docs/book/src/explanation/*.md` surface the same content in rustdoc and mdbook. **Dual-path documentation:** each algorithm has both a CPU (Rust in `adjust/`) and GPU (WGSL in `shaders/`) implementation. The sibling `.md` file documents the algorithm once — it is implementation-agnostic prose describing the math and design choices. Each explanation page includes a "Source" section listing both the Rust and WGSL source locations. The algorithm is documented once, not twice; the two implementations are noted as following the same math. A contributor-facing `crates/agx/src/engine/gpu/README.md` documents the dual-path architecture, how to add new adjustments to both paths, and the `GpuParameters` ↔ WGSL `Params` struct mapping. The sub-project also added structured WGSL headers, PR `book-linkcheck`, and the external linkcheck workflow. | Complete |
...
| 6 | **Tutorials** | One Getting Started page (`apply` then `edit`, layered) under `docs/book/src/tutorials/getting-started.md`. Install split out as a top-level `docs/book/src/install.md` page rather than a tutorial. The original `batch-apply` and `multi-apply` tutorial entries moved to sub-project #7 (how-to). See [Tutorials and How-to design](2026-04-26-tutorials-and-how-to-design.md) for the full audience-driven scope rewrite | Independent after #2 |
| 7 | **How-to guides** | Six recipes: apply preset to folder (`batch-apply`), compare looks side-by-side (`multi-apply`), write a preset, extend a preset, author a custom `.cube` LUT, compose layered looks. Each is task-focused and assumes the reader knows why they're there. "Match the output of another preset" cut and deferred to a future Recipes sub-project (genre-driven, spirit-not-numerics). See [Tutorials and How-to design](2026-04-26-tutorials-and-how-to-design.md) | Independent after #2 |
...
- **Sub-project #4:** Resolved by the algorithm explanations branch. Explanation depth varies by algorithm complexity; canonical prose lives in sibling `.md` files; citations use same-file footnotes where useful; WGSL files use structured headers that link each shader to the canonical explanation and CPU equivalent.
