...
| **Explanation** (algorithms) | Understanding-oriented | How grain / dehaze / denoise / detail / color grading / tone curves / vignette work, and why. Each algorithm has a CPU (Rust) and GPU (WGSL) implementation that follow the same math | Sibling `.md` files next to each `crates/agx/src/adjust/*.rs` — canonical source. Pulled into rustdoc via `#![doc = include_str!("module.md")]`; included into mdbook via `{{#include ...}}`. Each explanation page notes the dual-path implementation and links to both source locations |
...
| GPU implementation notes | Contributor-facing docs explaining dual-path (CPU Rust + GPU WGSL) architecture, how to keep paths in sync, and `GpuParameters` ↔ WGSL `Params` mapping | `crates/agx/src/engine/gpu/README.md` for contributor reference; algorithm explanation pages note both implementations |
...
| 4 | **Algorithm explanations** | Sibling `.md` file next to each `crates/agx/src/adjust/*.rs` (grain, dehaze, denoise, detail, color grading, tone curves, vignette, per-pixel adjustments) containing the canonical prose. `#![doc = include_str!("module.md")]` wiring on the Rust side and `{{#include ...}}` wiring in `docs/book/src/explanation/*.md`. **Replaces the short placeholder `//!` lines that sub-project #2 adds to each adjust submodule** (grain.rs is the only one already on the include_str pattern; the others get a one-paragraph inline `//!` in #2 that #4 supersedes). For the per-pixel adjustments specifically, sub-project #4's brainstorm should consider whether to ship one combined explanation or separate per-adjustment explanations (shadows, highlights, contrast, etc.) — sub-project #2's spec recommends the latter. Content can draw on existing design docs under `docs/plans/` (e.g., `2026-03-23-grain-design.md`, `2026-03-21-dehaze-design.md`) as reference material, but the canonical explanation lives in the sibling `.md` file. **Dual-path documentation:** each algorithm has both a CPU (Rust in `adjust/`) and GPU (WGSL in `shaders/`) implementation. The sibling `.md` file documents the algorithm once — it is implementation-agnostic prose describing the math and design choices. Each explanation page includes a "Source" section listing both the Rust and WGSL source locations. The algorithm is documented once, not twice; the two implementations are noted as following the same math. A contributor-facing `crates/agx/src/engine/gpu/README.md` documents the dual-path architecture, how to add new adjustments to both paths, and the `GpuParameters` ↔ WGSL `Params` struct mapping. **If this sub-project introduces external links (paper citations, etc.), it must add the external linkcheck workflow per the Enforcement and CI section** | Independent after #2 |
...
- **Sub-project #4:** Whether every adjust module needs equal explanation depth, or whether some get deep dives and others get short overviews. How much math to include. Whether to reference academic papers inline or in a separate bibliography. **GPU/WGSL documentation boundary:** WGSL has no `include_str!` or doc-generation tooling, so the documentation-in-code philosophy applies to Rust but not WGSL. Algorithm-level prose lives in sibling `.md` files on the Rust/adjust side and covers both implementations. WGSL shader comments remain implementation-focused. Decide whether this is sufficient or whether WGSL files need additional structured comments (e.g., header blocks linking to the canonical `.md` file).
...
5. Every `crates/agx/src/adjust/*.rs` module has a `//!` explanation block that is surfaced on the mdbook site. Algorithm explanation pages note the dual CPU/GPU implementation and link to both source locations.
6. `crates/agx/src/engine/gpu/README.md` documents the dual-path architecture for contributors.
7. `scripts/verify.sh` runs `cargo doc` as a gate and the site's broken-link checker passes.
8. `ARCHITECTURE.md` and `README.md` link to the site for expanded content but retain short summaries for repo-level discoverability.
