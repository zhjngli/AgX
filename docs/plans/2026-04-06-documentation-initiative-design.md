...
- `mdbook-linkcheck` runs on every PR; broken site links fail CI.
...
| 4 | **Algorithm explanations** | `//!` module doc blocks in each `crates/agx/src/adjust/*.rs` file (grain, dehaze, denoise, detail, color grading, tone curves, vignette, per-pixel adjustments). `{{#rustdoc_include}}` wiring in `docs/book/src/explanation/*.md`. Content can draw on existing design docs under `docs/plans/` (e.g., `2026-03-23-grain-design.md`, `2026-03-21-dehaze-design.md`) as reference material, but the canonical explanation lives in code | Independent after #2 |
| 5 | **Conceptual reference refresh** | Move `docs/reference/{color-spaces,grain-algorithm,lut-format}.md` into `docs/book/src/reference/concepts/`. Refresh "oxiraw" → "AgX" (may already be done as prep work). Expand with new topics: photographic terminology, preset compositional model, render pipeline conceptual overview, image processing basics | Independent after #2 |
