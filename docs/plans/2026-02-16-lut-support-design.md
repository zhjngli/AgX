...
**Future:** When we add wider color space support and a pluggable pipeline architecture, LUTs can declare their expected input/output color space, and the pipeline can auto-insert conversions. See `docs/backlog/pluggable-pipeline.md` for the pluggable pipeline vision.
