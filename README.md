...
AgX is a **preset-first** photo editing tool. It doesn't try to out-render Lightroom or Capture One — they own decades of proprietary color science. AgX's bet is different: the **portable, human-readable preset language is the product**, and its value grows with the ecosystem around looks you can read, diff, share, and fork as easily as code.
...
- **CLI and API first**: AgX is a library and a command-line tool. The core value lives in the portable preset format and the engine that renders it.
